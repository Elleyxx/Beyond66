...
const loadCurrentWeather = async () => {
  try {
    const weather = await getCurrentWeather(55.6761, 12.5683)
...
const denmarkData = computed(() => {
  const localized = tm('countries.denmark') || {}
  if (localized && typeof localized === 'object' && Object.keys(localized).length) return localized
  return getLocaleMessage('en')?.countries?.denmark || {}
})

const pageData = computed(() => {
  const data = denmarkData.value || {}
